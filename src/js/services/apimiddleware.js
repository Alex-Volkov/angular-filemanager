(function(angular) {
    ...
    angular.module('FileManagerApp').service('apiMiddleware', ['$window', 'fileManagerConfig', 'apiHandler', 
        ...
        ApiMiddleware.prototype.rename = function(item) {
            ...
            return this.apiHandler.rename(fileManagerConfig.renameUrl, itemPath, newPath, item.model.id);
        };

        ApiMiddleware.prototype.getUrl = function(item) {
            var itemPath = this.getFilePath(item);
            return this.apiHandler.getUrl(fileManagerConfig.downloadFileUrl, itemPath, item.model.id);
        };
